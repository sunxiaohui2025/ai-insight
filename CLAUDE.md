...
# One-click start (backend :3002 + frontend :3000), from project root
./start-all.sh          # start
./stop-all.sh           # stop

# Docker (production-style, binds 127.0.0.1:8000)
...
| `INSIGHT_LLM_MODEL` | `your-model-name` | Model name |
...
### One-click startup (project root)

| Script | 说明 |
|--------|------|
| `./start-all.sh` | 启动后端(:3002, 读取 `server/.env.local`) + 前端(:3000) |
| `./stop-all.sh` | 停止后端 + 前端 |
